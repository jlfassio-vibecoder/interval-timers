...
const WarmUpWheel: React.FC<WarmUpWheelProps> = ({
  ...
  const segmentProgress =
    activeIndex < exercises.length
      ...
      : 100;
  const translateY = -(activeIndex * (ITEM_HEIGHT + ITEM_GAP));
            ...
            >
              <div className="flex w-full items-center justify-center px-4">
                <div className="min-w-0 flex-1 text-center">
                  <h3
                    className={`font-display m-0 text-lg font-bold ${isActive ? 'text-[#ffbf00]' : 'text-white'}`}
                  >
                    {exercise.name}
                  </h3>
                  {exercise.detail && (
                    <span
                      className={`mt-1 block text-sm ${isActive ? 'text-[#ffbf00]/80' : 'text-white/70'}`}
                    >
                      {exercise.detail}
                    </span>
                  )}
