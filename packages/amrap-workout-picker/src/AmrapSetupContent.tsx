...
import type { RecentCustomWorkout } from './recentCustomWorkouts';
import type { AmrapLevel } from './amrap-setup-data';
import type { CustomExercise } from './useAmrapSetup';
import { isValidQty, formatQtyHint } from './exerciseFormat';
  ...
  return (
    <div className="space-y-6">
      ...
      {customExercises.length === 0 ? (
        <p className="text-sm text-white/50">Add at least one exercise to continue.</p>
      ) : onReorderExercises ? (
      ...
      <button
        ...
        className="w-full rounded-xl bg-orange-600 px-6 py-4 text-lg font-bold text-white shadow-[0_0_20px_rgba(234,88,12,0.3)] transition-all hover:bg-orange-500 disabled:cursor-not-allowed disabled:opacity-50"
      >
        Launch Custom AMRAP
      </button>
    </div>
