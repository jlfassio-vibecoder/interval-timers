...
| Non-PUBLIC_ env in client | OK | New/modified client code (`AppAnalyticsView`, admin views) does not use `import.meta.env`. API routes and server libs use `GOOGLE_PROJECT_ID`, `GEMINI_API_KEY`, etc. only in `src/pages/api/` and server-only modules — not exposed to client bundles. |
