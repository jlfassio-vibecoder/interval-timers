-- Trainer Live EMOM: prep is 10s setup only (matches SETUP_DURATION_SECONDS in app). skip_prep jumps past that slice.
...
BEGIN
  ...
  SELECT s.trainer_user_id, es.started_at
  INTO v_tl, v_started
  ...
  SET
    started_at = clock_timestamp() - (10 * interval '1 second'),
    paused_at = NULL,
    pause_accum_ms = 0
