 ...
 *
 * Public challenge sales view: Week 1 free, Weeks 2+ locked behind paywall.
 */
...
import type { ChallengePreviewData } from '@/types/ai-challenge';
...
export interface ChallengeSalesViewProps {
...
type MissionParamsWorkout = {
  workout: ChallengePreviewData['previewWeek']['workouts'][number];
  workoutIndex: number;
};

const ChallengeSalesView: React.FC<ChallengeSalesViewProps> = ({ challenge, zones = [] }) => {
  ...
  const { metadata, previewWeek, totalWeeks } = challenge;
  ...
  const handleUnlock = async () => {
    ...
    try {
      const res = await fetch('/api/challenges/unlock', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ challengeId: metadata.id }),
      });
      if (res.ok) {
      ...
      if (res.status === 401) {
        setUnlockError('Please log in to unlock this challenge.');
        return;
      }
    ...
    <div className="mx-auto max-w-7xl px-4 py-10 md:px-6">
      <header className="mb-12">
        ...
        <LandingPageDescription
          ...
          sectionImages={metadata.sectionImages}
          metadata={{
      ...
      {missionParamsWorkout != null && (
        <WorkoutDetailModal
          ...
          onLogWorkout={() => {
            // In the Week 1 free preview flow, "Log workout" should not implicitly unlock paid content.
            // Explicit unlock remains on the dedicated CTA button below.
      ...
      {lockedWeeksCount > 0 && (
        <div className="border-orange-light/30 bg-orange-light/10 rounded-xl border p-6 text-center">
          ...
          >
            ...
            {unlockLoading ? 'Unlocking…' : `Unlock Full ${totalWeeks}-Week Challenge`}
          </button>
        </div>
      )}
    </div>
...
export default ChallengeSalesView;
