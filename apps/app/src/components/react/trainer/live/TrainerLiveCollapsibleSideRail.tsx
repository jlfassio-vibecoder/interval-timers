...
/**
 * Generic right-edge collapsible rail (pattern from `SessionMessageBoardDrawer` in amrap
 * and `TrainerLiveVideoFeedDrawer`). Children stay mounted when collapsed.
 * With `sessionId`, open/closed is remembered for this live session (sessionStorage).
 */
export default function TrainerLiveCollapsibleSideRail({
  children,
  sessionId,
  defaultOpen = false,
  label,
  ...
  return (
    ...
    >
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
        aria-label={open ? ariaLabelCollapse : ariaLabelExpand}
