...
const StudioWelcomeEditorView: React.FC = () => {
  const [tab, setTab] = useState<StudioWelcomeLocaleKey>('en');
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [savedMsg, setSavedMsg] = useState<string | null>(null);
  const [studioLinked, setStudioLinked] = useState(false);
  const [studioSlug, setStudioSlug] = useState<string | null>(null);
  const [studioDisplayName, setStudioDisplayName] = useState<string | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch('/api/trainer/studio/welcome-content', { credentials: 'include' });
      ...
      setStudioLinked(!!data.studioLinked);
      setStudioSlug(data.slug ?? null);
      setStudioDisplayName(data.displayName ?? null);
  ...
  const onSave = async () => {
    ...
    setSavedMsg(null);
    try {
      const res = await fetch('/api/trainer/studio/welcome-content', {
        method: 'PATCH',
        credentials: 'include',
    ...
    <div className="mx-auto max-w-3xl text-white">
      <div className="mb-8">
        ...
        <p className="mt-2 max-w-2xl text-sm text-white/55">
          Customize text on the public invite and welcome pages for your studio. Leave fields empty
          to use the app default for that language.
        </p>
      </div>

      {!studioLinked ? (
        <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 px-5 py-4 text-sm text-amber-100/90">
          <p className="font-semibold">No studio linked</p>
          <p className="mt-2 text-white/70">
            Your coach record does not have a <code className="text-xs">studio_id</code> yet. Link a
            studio row in the database and set{' '}
            <code className="text-xs">public.trainers.studio_id</code> (or ask an admin) before
            saving org-level copy here.
          </p>
        </div>
      ) : (
        <p className="mb-6 text-sm text-white/50">
          Studio: <span className="text-white/80">{studioDisplayName}</span>{' '}
          <span className="text-white/35">({studioSlug})</span>
        </p>
      )}
      ...
      <section className="mb-10 rounded-2xl border border-white/10 bg-white/5 p-6">
        ...
        <ul className="mt-3 list-inside list-disc space-y-2 text-sm text-white/55">
          ...
          <li>
            The app passes <code className="text-xs">redirectTo</code> /{' '}
            <code className="text-xs">emailRedirectTo</code> to that URL when sending; template
            variables depend on your Supabase project settings.
          </li>
        </ul>
        ...
        <textarea
          ...
          disabled={!studioLinked}
          maxLength={WELCOME_LIMITS.emailSenderNote}
          rows={4}
          className="mt-2 w-full rounded-lg border border-white/15 bg-black/40 px-3 py-2 text-sm text-white placeholder:text-white/30 disabled:opacity-50"
...
export default StudioWelcomeEditorView;
