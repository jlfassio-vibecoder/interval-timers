...
// Use HIIT Workout Timer Supabase (same as AMRAP).
// Client bundles must only consume PUBLIC_* / VITE_* values.
...
if (!import.meta.env.DEV && isPlaceholder) {
  throw new Error(
    'Missing Supabase env in production. Set VITE_SUPABASE_* or PUBLIC_SUPABASE_*. See docs/SUPABASE_ENV.md'
  );
}
...
if (import.meta.env.DEV && isPlaceholder) {
  console.error(
    'Missing Supabase env. Set VITE_SUPABASE_* or PUBLIC_SUPABASE_* (HIIT project). See docs/SUPABASE_ENV.md'
  );
}
