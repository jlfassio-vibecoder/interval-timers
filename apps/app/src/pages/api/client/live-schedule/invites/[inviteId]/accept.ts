 ...
 *
 * POST accept a scheduled live session invite (capacity + waitlist in RPC).
 */
...
export const POST: APIRoute = async ({ request, cookies, params }) => {
  ...
  const { data, error } = await auth.supabase.rpc('trainer_live_schedule_invite_accept', {
    p_invite_id: inviteId,
  });

  if (error) {
    if (import.meta.env.DEV) console.error('[live-schedule accept]', error);
  ...
  if (!payload?.ok) {
    const msg = typeof payload?.error === 'string' ? payload.error : 'Accept failed';
    const full = msg.includes('full') || msg.includes('not pending');
