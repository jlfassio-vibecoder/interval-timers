/**
 * @license
 * SPDX-License-Identifier: Apache-2.0
 *
 * PATCH cancel a scheduled live invite (trainer-owned occurrence).
 */

import type { APIRoute } from 'astro';
import { verifyRosterAccessRequest } from '@/lib/supabase/admin/auth';
import { cancelLiveScheduleInvite } from '@/lib/supabase/admin/trainer-live-scheduled';

export const PATCH: APIRoute = async ({ request, cookies, params }) => {
  try {
    const { uid: viewerId } = await verifyRosterAccessRequest(request, cookies);
    const inviteId = params.inviteId?.trim();
    if (!inviteId) {
      return new Response(JSON.stringify({ error: 'inviteId required' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' },
      });
    }

    let body: { action?: string } = {};
    try {
      body = (await request.json()) as typeof body;
    } catch {
      return new Response(JSON.stringify({ error: 'Invalid JSON body' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' },
      });
    }

    const action = typeof body.action === 'string' ? body.action.trim() : '';
    if (action !== 'cancel') {
      return new Response(JSON.stringify({ error: 'action must be "cancel"' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' },
      });
    }

    const result = await cancelLiveScheduleInvite(viewerId, inviteId);
    if (!result.ok) {
      const nf = result.error.includes('not found');
      const bad = result.error.includes('Only pending');
      return new Response(JSON.stringify({ error: result.error }), {
        status: nf ? 404 : bad ? 400 : 400,
        headers: { 'Content-Type': 'application/json' },
      });
    }

    return new Response(JSON.stringify({ ok: true }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' },
    });
  } catch (error) {
    if (error instanceof Error) {
      if (error.message === 'UNAUTHENTICATED' || error.message === 'UNAUTHORIZED') {
        return new Response(
          JSON.stringify({ error: 'Unauthorized. Mission Control access required.' }),
          { status: 401, headers: { 'Content-Type': 'application/json' } }
        );
      }
    }
    if (import.meta.env.DEV || import.meta.env.PUBLIC_ENABLE_ERROR_LOGGING === 'true') {
      console.error('[trainer/live-schedule/invites PATCH]', error);
    }
    return new Response(JSON.stringify({ error: 'Failed to update invite' }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    });
  }
};
