 ...
 *
 * POST create a scheduled live occurrence + pending invites (roster clients only, P0).
 */

import type { APIRoute } from 'astro';
import { verifyRosterAccessRequest } from '@/lib/supabase/admin/auth';
import { createLiveScheduleOccurrence } from '@/lib/supabase/admin/trainer-live-scheduled';
  ...
  try {
    ...
    const ianaTimezone =
      typeof body.ianaTimezone === 'string' && body.ianaTimezone.trim()
        ? body.ianaTimezone.trim()
        : 'UTC';

    const url = new URL(request.url);
    const publicAppBaseHint = `${url.protocol}//${url.host}`;

    const result = await createLiveScheduleOccurrence(viewerId, role, {
      scheduledStartAt,
      scheduledEndAt,
      inviteeUserIds,
      prospectEmails,
    ...
    return new Response(JSON.stringify({ occurrenceId: result.occurrenceId }), {
      status: 201,
      headers: { 'Content-Type': 'application/json' },
    });
  } catch (error) {
    ...
    if (import.meta.env.DEV || import.meta.env.PUBLIC_ENABLE_ERROR_LOGGING === 'true') {
      console.error('[trainer/live-schedule/occurrences POST]', error);
    }
    return new Response(JSON.stringify({ error: 'Failed to create occurrence' }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    });
  }
