-- Trainer leaves interval UI (e.g. AMRAP embed) and returns to Video + intervals layout
-- (countdown_timer shell, no interval wrapper). Does not change shell to video_only.
-- Does not end the Trainer Live session, modify amrap_sessions, or finish AMRAP workouts.
...
GRANT EXECUTE ON FUNCTION public.trainer_live_return_to_main_video(uuid) TO authenticated;
