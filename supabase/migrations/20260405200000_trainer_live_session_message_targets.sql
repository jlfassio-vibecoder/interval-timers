...
ALTER TABLE public.trainer_live_session_messages
  ADD COLUMN IF NOT EXISTS target_participant_id uuid NULL
    REFERENCES public.trainer_live_participants (id) ON DELETE SET NULL;

COMMENT ON COLUMN public.trainer_live_session_messages.target_participant_id IS
  'Optional participant this message is directed at; validated in trainer_live_post_session_message.';
  ...
  RETURN COALESCE(
    (
      ...
      FROM (
        ...
        LEFT JOIN public.trainer_live_participants tgt ON tgt.id = m.target_participant_id
...
DROP FUNCTION IF EXISTS public.trainer_live_post_session_message(uuid, uuid, text, text);
...
DECLARE
  ...
  v_target uuid := p_target_participant_id;
  v_id uuid;
...
BEGIN
  ...
  IF v_target IS NOT NULL THEN
    IF NOT EXISTS (
      SELECT 1
      FROM public.trainer_live_participants t
      WHERE t.id = v_target
        AND t.session_id = p_session_id
    ) THEN
      RAISE EXCEPTION 'Invalid target participant';
    END IF;
  END IF;

  INSERT INTO public.trainer_live_session_messages (
    session_id,
    participant_id,
    body,
    reaction_key,
  ...
  VALUES (p_session_id, p_participant_id, v_body, v_rx, v_target)
  ...
  RETURN jsonb_build_object(
    ...
    'body', v_body,
    'reaction_key', v_rx,
    'target_participant_id', v_target,
    'created_at', v_created
  );
END;
$$;

GRANT EXECUTE ON FUNCTION public.trainer_live_post_session_message(uuid, uuid, text, text, uuid) TO anon, authenticated;
